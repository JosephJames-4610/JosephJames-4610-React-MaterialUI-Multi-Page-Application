...
const MemoryCheck = () => {
  const [shuffleMemoryData, setShuffleMemoryData] = useState(true);
  const [prevClickIndex, setPrevClickIndex] = useState('Initial');
  const [currClickIndex, setCurrClickIndex] = useState('Initial');
  const [matchFound, setMatchFound] = useState(false);
  const [matchCheck, setMatchCheck] = useState(false);
  const [stackOneData, setStackOneData] = useState([]);
  const [triggerStackOne, setTriggerStackOne] = useState(true);
  const [stackTwoData, setStackTwoData] = useState([]);
  const [triggerStackTwo, setTriggerStackTwo] = useState(true);
  const [stackThreeData, setStackThreeData] = useState([]);
  const [triggerStackThree, setTriggerStackThree] = useState(true);
  const [stackFourData, setStackFourData] = useState([]);
  const [triggerStackFour, setTriggerStackFour] = useState(true);

  const getOrderedMemoryData = () => {
    return MEMORY_DATA.map((value) => { return { ...value, order: 1 }}).sort(({order: orderA}, {order: orderB}) => orderA - orderB)
    // return MEMORY_DATA.map((value) => { return { ...value, order: Math.random() }}).sort(({order: orderA}, {order: orderB}) => orderA - orderB)
  };
  ...
  const updateButtonData = (stackData, setStackData,
    buttonIndex, triggerStack, setTriggerStack,
    variantValue, disabled = false) => {
  ...
  const getStackButtonUpdate = (fromStack, buttonIndex, disabled = false)=> {
  ...
  const getValuesSameCheck = (prevFromStack, prevButtonIndex, currFromStack, currButtonIndex) => {
  ...
  const handleButtonClick = (fromStack, buttonIndex) => {
    if (prevClickIndex !== 'Initial') {
  ...
  const renderStackButtons = (fromStack, stackData) => {
